...
export const checkTaxUpdatesWithAI = async (currentData: typeof ALIQUOTAS_DATA): Promise<{ hasUpdates: boolean, updatedData?: typeof ALIQUOTAS_DATA }> => {
    try {
        ...
        const prompt = `
            ...
            Dados Completos Atuais: ${JSON.stringify(currentData)}

            Se houver novas leis ou decretos federais importantes publicados após as datas de 'lastUpdated' que alterem as alíquotas padrão mostradas:
            1. Retorne um JSON com a propriedade "hasUpdates": true.
            2. Inclua a propriedade "updatedData" contendo o array completo dos dados, idêntico ao fornecido no prompt (Dados Completos Atuais), mas com as alíquotas corrigidas, as descrições ajustadas para refletir a nova legislação, e o campo 'lastUpdated' atualizado para a data de hoje.
            
            IMPORTANTE: O campo 'updatedData' deve manter a estrutura exata do objeto original (name, shortName, description, sections[], source, lastUpdated).

            Se não houver mudanças relevantes na legislação tributária federal/estadual (Convênios ICMS) para autopeças que afetem estes dados, retorne apenas {"hasUpdates": false}.
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                responseMimeType: "application/json",
                // Removido responseSchema devido à complexidade polimórfica do campo 'content' em sections,
                // o que causava erro de validação (empty properties for OBJECT type).
            }
        });
